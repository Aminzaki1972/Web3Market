...
(function(){
  ...
  async function render(){
    ...
    try{
      ...
      var data=(Array.isArray(q.data)?q.data:[]).filter(function(p){var s=String(p?.status||'').trim().toLowerCase();return (s==='active'||s==='sold')&&String(p?.project_status||'').trim().toLowerCase()==='approved'&&String(p?.ai_status||'').trim().toLowerCase()==='approved';}).slice(0,20);
      var old=document.getElementById('wm-live-projects-heading');
      if(!old){old=document.createElement('div');old.id='wm-live-projects-heading';old.innerHTML='<div style="display:flex;align-items:end;justify-content:space-between;gap:16px;margin:0 0 18px"><div><div style="font-size:11px;font-weight:950;letter-spacing:1px;color:#635bff">WEB3MARKET LISTINGS</div><h2 style="margin:5px 0 4px;font-size:32px;letter-spacing:-1px">Projects available and completed</h2><p style="margin:0;color:#737b88;font-size:13px">Approved, AI-reviewed projects are displayed below. Sold projects remain visible as permanent transaction records.</p></div><div style="font-size:13px;font-weight:900;color:#5149db">'+data.length+' project'+(data.length===1?'':'s')+'</div></div>';grid.parentNode.insertBefore(old,grid);}else{var count=old.querySelector('div[style*="color:#5149db"]');if(count)count.textContent=data.length+' project'+(data.length===1?'':'s');}
