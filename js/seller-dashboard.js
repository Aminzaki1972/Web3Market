...
(async function(){
 ...
 const {data:projects,error}=await sb.from('projects').select('id,title,status,price,currency,created_at').eq('owner_id',user.id).order('created_at',{ascending:false});
 ...
 const initials=(user.user_metadata?.name||user.email||'S').split(/[\s@._-]+/).filter(Boolean).slice(0,2).map(x=>x[0]).join('').toUpperCase();
 const name=esc(user.user_metadata?.name||'Web3 Seller');
 ...
 <div class="panel"><h2>Seller Profile</h2><div class="profile"><div class="avatar">${esc(initials)}</div><div><h3>${name}</h3><div class="muted">${email}</div><span class="badge">Seller Account</span></div></div></div>
 ...
 <div class="panel"><h2>Wallet & Payouts</h2><p class="muted">Your dashboard is ready for the Web3 payout layer. Connect a wallet before enabling live withdrawals.</p><a class="seller-btn" href="wallet.js">Manage Wallet</a></div>
 </aside></section>`;
})();
