...
(async function(){
 ...
 root.innerHTML=`<div class="card"><h2>Your listings (${projects.length})</h2>${projects.map(p=>`<article><h3><a href="project.html?id=${encodeURIComponent(p.id)}">${esc(p.title)}</a></h3><p>${esc(p.status)} · ${money(p.price,p.currency)}</p></article>`).join('')||'<p>No projects listed yet.</p>'}</div><div class="card"><h2>Incoming offers / deals (${deals.length})</h2>${deals.map(d=>`<p><a href="project.html?id=${encodeURIComponent(d.project_id)}">${esc(byId.get(d.project_id)?.title||'Project')}</a> — ${money(d.amount,d.currency)} — <strong>${esc(d.status)}</strong></p>`).join('')||'<p>No incoming offers yet.</p>'}</div>`;
})();
