"use strict";
(async function(){
  const root=document.getElementById("sellerGrid");
  if(!root)return;
  ...
  try{
    ...
    if(pr.error||!pr.data){error("Unable to load your seller profile.");return;}
    const profile=pr.data;
    if(String(profile.role||"").toLowerCase()!=="seller"){error("This account is not a Seller account.");return;}
    ...
    if(pq.error){error("Unable to load your listings.");return;}
