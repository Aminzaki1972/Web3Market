...
async function restoreDirectSession(){
  ...
  try{
    ...
    const result=await s.auth.setSession({access_token:access,refresh_token:refresh});
    if(result?.data?.session?.user)return result.data.session;
  }catch(e){console.warn("Web3Market session restore:",e)}
  return null;
}
window.Web3MarketSupabaseRestoreSession=restoreDirectSession;
...
function loadRealMarketplace(){if(!document.querySelector('.listingGrid')||document.getElementById('web3market-real-marketplace'))return;const s=document.createElement('script');s.id='web3market-real-marketplace';s.src='js/real-marketplace.js?v=20260914-active-clean';s.async=true;s.onerror=function(){console.warn('Web3Market real marketplace unavailable')};document.body.appendChild(s)}
