...
(async function(){
 ...
 if(!sb?.auth){root.innerHTML='<div class="status">Verification panel unavailable. The seller wallet connection is separate and can still be used above.</div>';return;}
 try{
  if(window.Web3MarketSupabaseRestoreSession)await window.Web3MarketSupabaseRestoreSession();
  const {data:{user},error:authError}=await sb.auth.getUser();
  if(authError||!user){root.innerHTML='<div class="status">Please sign in before requesting project verification.</div>';return;}
  ...
  if(error){root.innerHTML='<div class="status">Unable to load project verification data. Wallet ownership verification is handled separately above.</div>';return;}
 ...
 }catch(e){console.error('Web3Market verification:',e);root.innerHTML='<div class="status">Project verification is temporarily unavailable. The wallet verification area above is independent.</div>';}
})();
