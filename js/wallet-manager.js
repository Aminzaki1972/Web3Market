...
(function(){
  ...
  function getClient(){
    try{
      const existing=window.Web3MarketSupabase?.getClient?.()||window.Web3MarketSupabase?.client||window.supabaseClient||window.web3marketSupabase;
      ...
      if(window.supabase?.createClient){
        ...
        window.Web3MarketSupabase.client=c;
        window.Web3MarketSupabase.supabase=c;
        window.Web3MarketSupabase.getClient=()=>c;
        window.supabaseClient=c;
        window.web3marketSupabase=c;
        return c;
      }
    }catch(e){console.warn("Web3Market wallet client init:",e)}
    return null;
  }
  ...
  async function restoreSession(c){
    try{
      ...
      if(typeof window.Web3MarketSupabaseRestoreSession==="function"){
        s=await window.Web3MarketSupabaseRestoreSession();
        if(s?.access_token)return s;
      }
  ...
  function addProvider(provider,info){
    ...
    seen.add(provider);
    providers.push({provider,info:info||{}});
  }
  function discover(){
    try{
      if(window.safepalProvider)addProvider(window.safepalProvider,{name:"SafePal",rdns:"com.safepal.wallet"});
      if(window.okxwallet)addProvider(window.okxwallet,{name:"OKX Wallet",rdns:"com.okex.wallet"});
      const tw=window.trustwallet?.ethereum||window.trustwallet;
      if(tw?.request)addProvider(tw,{name:"Trust Wallet",rdns:"com.trustwallet.app"});
      const eth=window.ethereum;
      if(eth?.providers?.length)eth.providers.forEach(p=>addProvider(p,p.info||{}));
      else if(eth)addProvider(eth,eth.info||{});
      window.dispatchEvent(new Event("eip6963:requestProvider"));
    }catch(e){console.warn("Web3Market wallet discovery:",e)}
  }
  ...
  function providerName(x){
    const s=String(x.info?.name||x.info?.rdns||x.provider?.name||"").toLowerCase();
  ...
  function launch(name,notice){
    const url=deepLink(name);
    if(!url){if(notice)notice.textContent="Open "+name+" and use its built-in browser to visit Web3Market.";return false}
    if(notice)notice.textContent="Opening "+name+"…";
    try{
      ...
      return true;
    }catch(e){try{window.location.assign(url);return true}catch(_){return false}}
  }
  ...
  async function connectAndVerify(provider,walletName,opts={}){
    ...
    const purpose=String(opts.purpose||"wallet_ownership");
    const role=String(opts.role||"user").toLowerCase();
    const setNotice=typeof opts.setNotice==="function"?opts.setNotice:()=>{};
    ...
    if(chain!==BSC){
      try{await provider.request({method:"wallet_switchEthereumChain",params:[{chainId:BSC}]})}
        ...
        if(e?.code===4902){
          await provider.request({method:"wallet_addEthereumChain",params:[{chainId:BSC,chainName:"BNB Smart Chain",nativeCurrency:{name:"BNB",symbol:"BNB",decimals:18},rpcUrls:["https://bsc-dataseed.binance.org/"],blockExplorerUrls:["https://bscscan.com/"]}]});
        }else throw e;
      }
    ...
    if(!sb?.auth)throw new Error("Web3Market connection could not be initialized. Please refresh the page.");
    const session=await restoreSession(sb);
    if(!session?.access_token)throw new Error("Your Web3Market session is unavailable. Please sign in again.");
    const user=(await sb.auth.getUser())?.data?.user;
    if(!user?.id)throw new Error("Your Web3Market session is invalid. Please sign in again.");
    ...
    const message=[
      ...
      "Purpose: "+purpose,
      "Wallet: "+address,
      "Chain ID: "+BSC_DECIMAL+" (BNB Smart Chain)",
      "Issued At: "+timestamp,
      "",
    ...
    if(!response.ok||result.ok!==true||result.verified!==true)throw new Error(result.error||"Wallet verification failed.");
  ...
  function listWallets(){
    ...
    providers.forEach(x=>{const name=providerName(x);if(!used.has(name)){used.add(name);rows.push({name,provider:x.provider,detected:true,icon:x.info?.icon||""})}});
    preferred.forEach(name=>{if(!used.has(name)){used.add(name);rows.push({name,provider:null,detected:false,icon:""})}});
    return rows;
  }
