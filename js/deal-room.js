...
(async function(){
 ...
 async function prepareRelease(){
  const btn=document.querySelector('#prepareReleaseBtn');
  if(btn)btn.disabled=true;
  const box=document.querySelector('#releaseResult');
  if(box)box.innerHTML='<div class="notice">Validating payment, deal amount, 7.5% fee, seller wallet, Safe and token configuration…</div>';
  const {data,error}=await sb.functions.invoke('prepare-deal-release',{body:{deal_id:deal.id}});
  if(error||data?.error){console.error('prepare release',error,data);if(box)box.innerHTML=`<div class="notice warn"><strong>Release not ready.</strong><br>${esc(data?.error||error?.message||'Validation failed.')}</div>`;if(btn)btn.disabled=false;return}
  if(box)box.innerHTML=`<div class="notice ok"><strong>Release policy locked and validated.</strong><br>Seller receives ${esc(money(data.policy.seller_net_amount))}. Web3Market fee: ${esc(money(data.policy.fee_amount))}.<br><span>Safe must still collect the required 2-of-3 owner approvals.</span></div>`;
  await renderSafeStatus();
 }
 async function renderSafeStatus(){
  ...
  box.innerHTML=`<div class="safe-panel ${is23&&isContract?'ok':'warn'}"><strong>Safe 2-of-3 verification</strong><br><span>${isContract?'Contract detected':'Address is not a contract'} · ${owners.length} owner(s) · threshold ${threshold??'?'}</span><br><span>Safe nonce: ${esc(nonce??'—')}</span>${is23&&isContract?`<hr><div class="safe-release"><strong>Release target:</strong> Seller<br><strong>Seller net:</strong> ${esc(money(sellerNet))}<br><strong>Platform fee:</strong> ${esc(money(fee))}</div><div id="releaseResult"></div><button id="prepareReleaseBtn" class="btn primary">Validate & Prepare Release</button><a class="btn primary" href="${safeUrl}" target="_blank" rel="noopener noreferrer">Open Safe Queue</a><div class="safe-note">Preparation is server-validated. The actual Safe transaction still requires the required owners to sign; Web3Market cannot release funds alone.</div>`:'<div class="safe-note">Expected configuration is exactly 3 owners with a 2-signature threshold. No release action is enabled here when that condition is not verified.</div>'}</div>`;
  const prepare=document.querySelector('#prepareReleaseBtn'); if(prepare)prepare.onclick=prepareRelease;
 }
