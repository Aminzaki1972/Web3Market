...
function renderExternal(p){
 let h='<div class="passport-badge external">External Project • Not Listed on Web3Market</div><section class="passport-card"><h2>Free Public Project Research</h2><p class="muted">This external mode uses public sources only. No OpenAI API is used.</p><div class="identity">';
 [['Project','project_name'],['Website','website'],['Category','category'],['Launched','founded_or_launched'],['Technology','technology'],['GitHub','github'],['Users','active_users'],['Monthly Traffic','monthly_visits'],['Revenue','revenue'],['Risk Level','ai_risk_level'],['Risk Score','ai_risk_score'],['Confidence','confidence_score']].forEach(([l,k])=>h+='<div class="item"><small>'+l+'</small><strong>'+esc(val(p[k]))+'</strong>'+status('external','Public Source')+'</div>');
 h+='</div></section><section class="passport-card"><h2>Research Summary</h2><p>'+esc(val(p.ai_summary))+'</p></section><section class="passport-card"><h2>Key Findings</h2>';
 ...
 h+='</section><section class="passport-card"><p class="muted"><strong>Important:</strong> This is external public research only. It is not a Web3Market listing, is not owner-verified, and does not create or modify marketplace data.</p></section>';
 document.getElementById('passport').innerHTML=h;
}
...
async function loadExternal(query){
 const root=document.getElementById('passport');root.innerHTML='<p>Searching free public sources and building External Passport…</p>';
