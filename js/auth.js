...
(function(){
 ...
 async function upsertProfile(client,user,walletAddress,email,role,fullName){
  const selectedRole=normalizeRole(role);if(!client||!user||!selectedRole)return null;
  const profileData={id:user.id,email:normalizeEmail(email||user.email||"")||null,wallet_address:normalizeWalletAddress(walletAddress)||null,role:selectedRole,updated_at:new Date().toISOString()};
  if(fullName)profileData.display_name=String(fullName).trim();
  const result=await client.from(PROFILE_TABLE).upsert(profileData,{onConflict:"id"}).select("id,email,role,display_name,wallet_address").maybeSingle();
