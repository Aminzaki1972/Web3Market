...
(function(){
 ...
 async function syncProfileServerSide(client){
  const {data:{session},error:sessionError}=await client.auth.getSession();
  if(sessionError||!session?.access_token)throw sessionError||new Error("Authenticated session is unavailable.");
  const response=await fetch(COMPLETE_REGISTRATION_URL,{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+session.access_token},body:"{}"});
  const body=await response.json().catch(()=>({}));
  if(!response.ok||!body.success)throw new Error(body.error||"Profile synchronization failed.");
  return body;
 }
 async function persistRegistration(client,user,registration){
  const pending=registration||getPendingRegistration()||{};
  const role=normalizeRole(pending.role||user?.user_metadata?.role)||"buyer";
  const walletAddress=normalizeWalletAddress(pending.walletAddress||user?.user_metadata?.wallet_address||"");
  const email=normalizeEmail(pending.email||user?.email||"");
  const fullName=String(pending.fullName||user?.user_metadata?.full_name||"").trim();
  await updateUserMetadata(client,walletAddress,role,fullName);
  await syncProfileServerSide(client);
  clearPendingRegistration();
  return {success:true,role,user};
 }
 async function ensureProfileForAuthenticatedUser(){
  const client=getSupabaseClient();if(!client?.auth)return null;
  const {data:userData,error:userError}=await client.auth.getUser();if(userError)throw normalizeAuthError(userError);const user=userData?.user;if(!user)return null;
  const {data:existing,error:profileError}=await client.from("profiles").select("id,email,role,display_name,wallet_address").eq("id",user.id).maybeSingle();
  if(!profileError&&existing?.role&&normalizeRole(existing.role))return existing;
  const pending=getPendingRegistration()||{};const role=normalizeRole(pending.role||user.user_metadata?.role)||"buyer";
  const walletAddress=normalizeWalletAddress(pending.walletAddress||user.user_metadata?.wallet_address||"");
  const fullName=String(pending.fullName||user.user_metadata?.full_name||"").trim();
  await updateUserMetadata(client,walletAddress,role,fullName);
  await syncProfileServerSide(client);
  return (await client.from("profiles").select("id,email,role,display_name,wallet_address").eq("id",user.id).maybeSingle()).data||null;
 }
 async function registerWithWallet(registration){
  ...
  if(data?.session){const {error:sessionError}=await client.auth.setSession({access_token:data.session.access_token,refresh_token:data.session.refresh_token});if(sessionError)throw normalizeAuthError(sessionError);}
  const {data:authData,error:authError}=await client.auth.getUser();if(authError)throw normalizeAuthError(authError);const user=authData?.user;if(!user)throw new Error("Wallet authentication did not return a user session.");
  ...
  return {success:true,requiresEmailVerification:false,walletAddress,role,user,session:data.session||await getSession()};
 }
