...
(function(){
  ...
  function collect(){
    const p={};
    form.querySelectorAll('input[name],textarea[name],select[name]').forEach(el=>{if(['project_types','services','audience','assets'].includes(el.name))return;p[el.name]=el.type==='number'?normalizeNumber(el.value):el.value;});
    ...
    p.performance={users_count:normalizeNumber(p.users_count),active_users:normalizeNumber(p.active_users),customers_count:normalizeNumber(p.customers_count),monthly_visits:normalizeNumber(p.monthly_visits),total_sales:normalizeNumber(p.total_sales),monthly_volume:normalizeNumber(p.monthly_volume),growth_rate:normalizeNumber(p.growth_rate),conversion_rate:p.conversion_rate,last_active_date:normalizeDate(p.last_active_date),traffic_sources:p.traffic_sources};
    p.financials={has_revenue:p.has_revenue,revenue_period:p.revenue_period,monthly_revenue:normalizeNumber(p.monthly_revenue),yearly_revenue:normalizeNumber(p.yearly_revenue),monthly_net_profit:normalizeNumber(p.monthly_net_profit||p.monthly_profit),yearly_net_profit:normalizeNumber(p.yearly_net_profit||p.yearly_profit),monthly_expenses:normalizeNumber(p.monthly_expenses),growth_rate:normalizeNumber(p.growth_rate),revenue_sources:p.revenue_sources,financial_notes:p.financial_notes};
    p=normalizePayload(p);
    p.price=normalizeNumber(value('asking_price')||p.asking_price||p.price);
    p.asking_price=normalizeNumber(p.asking_price);
    p.currency=value('currency')||'USD';
    p.status='draft';
    return p;
  }
  ...
  function loadLocalDraft(){try{const raw=localStorage.getItem(LOCAL_DRAFT_KEY);if(!raw)return false;const x=JSON.parse(raw);if(!x?.data)return false;fill(x.data);if(out)out.textContent='Recovered unsaved form data from this device.';return true;}catch(e){console.warn('Local draft load failed',e);return false;}}

  async function loadDraft(){
    const c=await waitClient();
    if(!c){loadLocalDraft();if(!value('title')&&out)out.textContent='Database connection unavailable.';return;}
    const u=await c.auth.getUser();
    if(u.error||!u.data?.user){loadLocalDraft();if(!value('title')&&out)out.textContent='Please sign in before listing a project.';return;}
    ...
    if(p){currentProjectId=p.id;localStorage.setItem(PROJECT_ID_KEY,p.id);fill(p);localStorage.removeItem(LOCAL_DRAFT_KEY);if(out)out.textContent='Saved draft loaded successfully.';return;}
    if(!loadLocalDraft()&&out)out.textContent='No saved draft found. Your form is ready for a new listing.';
  }

  async function saveDraft(){
    const c=await waitClient();
    if(!c){saveLocalDraft();if(out)out.textContent='Database connection unavailable. Your form is safely saved on this device.';return null;}
    const u=await c.auth.getUser();
    if(u.error||!u.data?.user){saveLocalDraft();if(out)out.textContent='Please sign in before listing a project. Your form is saved on this device.';return null;}
    const uid=u.data.user.id;
    if(!value('title')||value('short_description').length<20||(value('full_description')||value('description')).length<50){saveLocalDraft();if(out)out.textContent='Please complete the required project information. Your current form is saved on this device.';return null;}
    const payload=normalizePayload(collect());payload.owner_id=uid;payload.status='draft';if(out)out.textContent='Saving draft…';
    let savedId=null;
    if(currentProjectId){const q=await c.from('projects').update(payload).eq('id',currentProjectId).eq('owner_id',uid).select('id');if(q.error){console.error(q.error);saveLocalDraft();if(out)out.textContent=(q.error.message||'Unable to save listing.')+' Your form is also saved on this device.';return null;}if(q.data?.length)savedId=q.data[0].id;}
    if(!savedId){const q=await c.from('projects').insert(payload).select('id');if(q.error){console.error(q.error);saveLocalDraft();if(out)out.textContent=(q.error.message||'Unable to create listing.')+' Your form is saved on this device.';return null;}if(q.data?.length)savedId=q.data[0].id;}
    if(!savedId){saveLocalDraft();if(out)out.textContent='The draft could not be confirmed by the database. Your form is safely saved on this device.';return null;}
    currentProjectId=savedId;localStorage.setItem(PROJECT_ID_KEY,savedId);localStorage.removeItem(LOCAL_DRAFT_KEY);if(out)out.textContent='Draft saved successfully.';return savedId;
  }

  async function submitForReview(){const id=await saveDraft();if(!id)return;const c=await waitClient();if(!c)return;if(out)out.textContent='Submitting for AI Review…';const r=await c.functions.invoke('ai-review-project',{body:{project_id:id}});if(r.error){if(out)out.textContent='AI Review could not start: '+(r.error.message||'unknown error');return;}const x=r.data?.result||{};if(out)out.innerHTML='AI Review completed. Score: <strong>'+(x.ai_score??0)+'/100</strong> — '+(x.ai_status||'completed')+'.';}
