...
(function(){
  ...
  const numericFields=['price','asking_price','year_created','monthly_revenue','yearly_revenue','monthly_profit','yearly_profit','monthly_net_profit','yearly_net_profit','monthly_expenses','total_sales','monthly_volume','active_users','customers_count','monthly_visits','users_count','ai_score'];
  ...
  function normalizeNumber(v){if(v===null||v===undefined)return null;if(typeof v==='number')return Number.isFinite(v)?v:null;const s=String(v).trim();if(s==='')return null;const n=Number(s);return Number.isFinite(n)?n:null;}
  ...
  function collect(){
    ...
    form.querySelectorAll('input[name],textarea[name],select[name]').forEach(el=>{
      if(['project_types','services','audience','assets'].includes(el.name))return;
      const raw=String(el.value??'').trim();
      p[el.name]=el.type==='number'?normalizeNumber(raw):raw;
    });
  ...
  async function saveDraft(){
    ...
    const payload=normalizePayload(collect());payload.owner_id=uid;out.textContent='Saving draft…';
    let savedId=null;
    if(currentProjectId){const q=await c.from('projects').update(payload).eq('id',currentProjectId).eq('owner_id',uid).select('id');if(q.error){console.error(q.error);saveLocalDraft();out.textContent=(q.error.message||'Unable to save listing.')+' Your form is also saved on this device.';return null;}if(q.data?.length)savedId=q.data[0].id;}
    if(!savedId){const q=await c.from('projects').insert(payload).select('id');if(q.error){console.error(q.error);saveLocalDraft();out.textContent=(q.error.message||'Unable to create listing.')+' Your form is saved on this device.';return null;}if(q.data?.length)savedId=q.data[0].id;}
