...
(async function(){
 ...
 const dealsResult=await sb.from('deals').select('id,project_id,amount,currency,status,created_at').eq('buyer_id',user.id).order('created_at',{ascending:false});
 ...
 root.innerHTML=`
 <div class="dash-top"><div><h1>Buyer Dashboard</h1><p>Discover, evaluate and acquire Web3 projects from one professional workspace.</p></div><div class="dash-actions"><a class="dash-btn" href="marketplace.html">Browse Marketplace</a><a class="dash-btn primary" href="marketplace.html">⌕ Find a Project</a></div></div>
 <div class="buyer-banner">✦ Your buyer workspace is ready — manage purchases, offers, deals and secure payments in one place.</div>
 ...
 <section class="stats"><div class="stat"><div class="label">TOTAL SPENT</div><div class="value">${money(spent,'USD')}</div><div class="sub">Completed purchases</div></div><div class="stat"><div class="label">PROJECTS BOUGHT</div><div class="value">${completed.length}</div><div class="sub">Successful acquisitions</div></div><div class="stat"><div class="label">OFFERS & DEALS</div><div class="value">${deals.length}</div><div class="sub">${pending.length} awaiting action</div></div><div class="stat"><div class="label">COMPLETED DEALS</div><div class="value">${completed.length}</div><div class="sub">Successful transactions</div></div></section>
 <section class="buyer-grid"><div>
 <div class="panel" id="purchases"><h2>My Purchases</h2>${completed.length?completed.slice(0,8).map(d=>{const p=byId.get(d.project_id);return `<article class="listing"><div><h3><a href="project.html?id=${encodeURIComponent(d.project_id||'')}">${esc(p?.title||'Project')}</a></h3><div class="muted">${money(d.amount,d.currency)} · ${d.created_at?new Date(d.created_at).toLocaleDateString():''}</div></div><span class="status-pill">Completed</span></article>`}).join(''):'<div class="empty">No completed purchases yet.<br><br><a class="dash-btn primary" href="marketplace.html">Browse Projects</a></div>'}</div>
 <div class="panel" id="deals"><h2>Offers & Deals</h2>${deals.length?deals.slice(0,8).map(d=>{const p=byId.get(d.project_id);return `<article class="listing"><div><h3>${p?`<a href="project.html?id=${encodeURIComponent(d.project_id||'')}">${esc(p.title||'Project')}</a>`:'Project'}</h3><div class="muted">${money(d.amount,d.currency)} · ${d.created_at?new Date(d.created_at).toLocaleDateString():''}</div></div><span class="status-pill">${esc(d.status||'pending')}</span></article>`}).join(''):'<div class="empty">No offers or deals yet. Browse the marketplace to find your next acquisition.</div>'}</div>
 ...
 <div class="panel" id="reviews"><h2>Buyer Reputation</h2><div class="score">${reputation}/100</div><div class="muted">Buyer reputation</div><div class="progress"><span style="width:${reputation}%"></span></div><div class="muted">Your reputation grows with successful transactions and verified reviews.</div></div>
