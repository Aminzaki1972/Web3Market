...
(async function(){
 ...
 async function renderSafe(){
  ...
  try{
   ...
   box.innerHTML=`<div class="safe-panel ${ok?'ok':'warn'}"><strong>Safe 2-of-3 verification</strong><br>${code!=='0x'?'Contract detected':'Not a contract'} · ${owners.length} owner(s) · threshold ${Number(threshold)}<br>Nonce: ${esc(nonce)}<div class="safe-note">${ok?'The Safe configuration is valid. Web3Market cannot release funds alone.':'Expected exactly 3 owners with a 2-signature threshold.'}</div>${ok?`<a class="btn primary" href="https://app.safe.global/transactions/queue?safe=bnb:${encodeURIComponent(safe)}" target="_blank" rel="noopener noreferrer">Open Safe Queue</a>${participant==='buyer'&&!Boolean(deal.payment_tx_hash)&&String(deal.payment_status||'').toLowerCase()!=='confirmed'&&/^0x[a-fA-F0-9]{40}$/.test(String(deal.token_contract||''))?`<a class="btn primary" href="deal-checkout.html?deal=${encodeURIComponent(deal.id)}" style="margin-top:8px">Pay ${esc(deal.expected_amount??deal.amount??0)} ${esc(deal.token_symbol||'USDT')}</a>`:''}:''}</div>`;
