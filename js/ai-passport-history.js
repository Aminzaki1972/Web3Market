...
async function renderExternal(p){
 ...
 graphSources.slice(0,20).forEach(x=>{const u=String(x.url||'');if(/^https?:\/\//i.test(u))graph+='<div class="source"><a href="'+esc(u)+'" target="_blank" rel="noopener noreferrer">'+esc(x.title||u)+'</a> <small>'+esc(x.type||'public')+'</small></div>';});graph+='</section>';const root=document.getElementById('passport');root.querySelectorAll('[data-passport-history]').forEach(x=>x.remove());root.insertAdjacentHTML('beforeend',graph); const snapshots=await snapshotHistoryUI(p);root.insertAdjacentHTML('beforeend',snapshots.replace('<section class="passport-card">','<section data-passport-history="snapshots" class="passport-card">'));const history=await historyUI(p);root.insertAdjacentHTML('beforeend',history.replace('<section class="passport-card">','<section data-passport-history="changes" class="passport-card">'));
}
