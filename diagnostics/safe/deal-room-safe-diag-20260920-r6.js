...
(async function(){
 ...
 async function renderReleaseSigning(){
  ...
  const selectSigningProvider=async()=>{
    ...
    const available=detected.filter(function(row){return row.provider;});
    if(!available.length){
      const preferred=detected.find(function(row){return String(row.name).toLowerCase()==='safepal';})||detected[0];
      if(preferred){ wm.launch(preferred.name,function(msg){});
        throw new Error('No wallet provider is currently available. The wallet app/browser was opened. Return to Web3Market and try Sign Safe Release again.');
      }
      throw new Error('No supported wallet provider was detected. Open the wallet linked to this deal and try again.');
    }
